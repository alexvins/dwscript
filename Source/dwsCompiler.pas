...
begin
   ...
   end else Result:=CreateMethodExpr(FProg, meth, Expr, RefKind, scriptPos, options);
...
begin
   ...
   if (bestMatch<>nil) and (bestCount=1) then begin
      ...
      if bestMatch<>funcExpr.FuncSym then begin
         ...
         funcExpr:=funcExpr.ChangeFuncSymbol(FProg, bestMatch, cfOptions);
      end;
      Result:=True;
   end else begin
      ...
      case token of
         ...
         ttPLUS_ASSIGN, ttMINUS_ASSIGN, ttTIMES_ASSIGN, ttDIVIDE_ASSIGN, ttCARET_ASSIGN : begin
            if left.Typ is TClassSymbol then begin
               ...
               classOpExpr:=GetMethodExpr(classOpSymbol.UsesSym, left, rkObjRef, scriptPos, []);
